    ...
    return (
            <div className="w-60 h-auto">
                ...
                <div className="flex gap-3 ">
                    <div className="text-red-500 font-medium">${Number(data.price - (data.price * (data.discount / 100))).toFixed(2)}</div>
                    {data.discount ? <div className="text-stone-500 line-through">${Number(data.price).toFixed(2)}</div> : null}
                </div>
            </div>
    )
}
